...
class ForgotFrame(tk.Frame):
    ...
    def _create_rounded_entry(self, x_pos, y_pos, placeholder="", show_char="", with_eye=False):
        round_rectangle(self.canvas, x_pos + 12, y_pos + 7, x_pos + 232, y_pos + 37, r=10, fill="#DDDDDD", outline="")
        round_rectangle(self.canvas, x_pos + 10, y_pos + 5, x_pos + 230, y_pos + 35, r=10, fill="white", outline="#CCCCCC", width=1)
    ...
    def on_get_code(self):
        ...
        for item in (self.get_code_btn_id, self.get_code_text_id):
            ...
            self.canvas.tag_bind(item, "<Enter>", lambda e: (self.canvas.itemconfig(self.get_code_btn_id, fill="#333333"), self.config(cursor="hand2")))
            self.canvas.tag_bind(item, "<Leave>", lambda e: (self.canvas.itemconfig(self.get_code_btn_id, fill="#000000"), self.config(cursor="")))
